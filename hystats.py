...
class stats(commands.Cog):
    ...
    async def hystats(self,ctx,msg=None):
        async with aiohttp.ClientSession() as session:
            ...
            'print(json)'
        if not json["success"]:
            await ctx.send('Error: ' + json["cause"])
        elif not json["player"] != None:
            await ctx.send('Error: that player does not exist')
        else:
            ...
            embed.add_field(name='Duels Stats', value=f'Games Played: {duels["games_played_duels"]}, Games Won: {str(int(duels["games_played_duels"] - duels["losses"]))}',inline=False)
            ...
            embed.add_field(name="Skywars Stats", value=f'Games Won: {skywars["games_played_skywars"]}, Games Won: {skywars["games_played_skywars"] - skywars["losses"]}, Souls: {skywars["souls"]}')
