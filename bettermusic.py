...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def search_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        cls.search["author"] = {'name': f'{ctx.author.name}', 'url': f'{ctx.author.avatar}',
        ...
        em.set_author(name='search results', icon_url=discord.Member.avatar)
        ...
        try:
            m = await cls.wait_for('message', check=check, timeout=45.0)
...
class Music(commands.Cog):
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        embed.set_author(name=self.requester.name, icon_url=self.requester.avatar)
        await ctx.send(embed=embed)
